...
@Entity data class Work(@PrimaryKey(autoGenerate=true) val id:Long=0,val name:String,val client:String="",val location:String="",val progress:Int=0)
@Entity data class Task(@PrimaryKey(autoGenerate=true) val id:Long=0,val workId:Long,val title:String,val stage:String="Geral",val done:Boolean=false)
@Entity data class Diary(@PrimaryKey(autoGenerate=true) val id:Long=0,val workId:Long,val text:String,val time:Long=System.currentTimeMillis())
@Entity data class Check(@PrimaryKey(autoGenerate=true) val id:Long=0,val workId:Long,val title:String,val done:Boolean=false)

@Dao interface WorkDao{@Query("SELECT * FROM Work ORDER BY id DESC") fun all():Flow<List<Work>>;@Insert suspend fun add(w:Work);@Update suspend fun update(w:Work)}
@Dao interface TaskDao{@Query("SELECT * FROM Task WHERE workId=:id ORDER BY id DESC") fun all(id:Long):Flow<List<Task>>;@Insert suspend fun add(t:Task);@Update suspend fun update(t:Task)}
...
@Dao interface CheckDao{@Query("SELECT * FROM Check WHERE workId=:id ORDER BY id") fun all(id:Long):Flow<List<Check>>;@Insert suspend fun add(c:Check);@Update suspend fun update(c:Check)}
...
@Composable fun Home(ws:List<Work>,open:(Work)->Unit){LazyColumn{item{Title("OBRA360","Controle prático da obra, mesmo offline.")};item{Row(Modifier.padding(16.dp),horizontalArrangement=Arrangement.spacedBy(8.dp)){Stat("Obras",ws.size);Stat("Ativas",ws.count{it.progress<100});Stat("Concluídas",ws.count{it.progress==100})}};item{Head("Obras recentes")};if(ws.isEmpty())item{Empty("Ainda não há obras","Crie uma obra para começar.")};items(ws){Card(it,open)};item{Spacer(Modifier.height(80.dp))}}}
...
@Composable fun Stat(t:String,n:Int){Surface(Modifier.weight(1f),shape=RoundedCornerShape(18.dp),color=Color.White){Column(Modifier.padding(13.dp)){Text(n.toString(),style=MaterialTheme.typography.headlineSmall,fontWeight=FontWeight.Bold);Text(t,color=Muted)}}}
...
@Composable fun WorkDialog(save:(List<String>)->Unit,cancel:()->Unit){var n by remember{mutableStateOf("")};var c by remember{mutableStateOf("")};var l by remember{mutableStateOf("")};AlertDialog(onDismissRequest=cancel,title={Text("Nova obra")},text={Column(verticalArrangement=Arrangement.spacedBy(8.dp)){OutlinedTextField(n,{n=it},label={Text("Nome")});OutlinedTextField(c,{c=it},label={Text("Cliente")});OutlinedTextField(l,{l=it},label={Text("Local")})}},confirmButton={Button(n.isNotBlank(),{save(listOf(n,c,l))}){Text("Criar")}},dismissButton={TextButton(cancel){Text("Cancelar")}})}
@Composable fun Detail(w:Work?,vm:VM,back:()->Unit){if(w==null){back();return};var page by rememberSaveable{mutableStateOf(0)};val ts by vm.tasks(w.id).collectAsState(initial=emptyList());val ds by vm.diary(w.id).collectAsState(initial=emptyList());val cs by vm.checks(w.id).collectAsState(initial=emptyList());var dialog by remember{mutableStateOf("")};Column{Row(Modifier.padding(10.dp),verticalAlignment=Alignment.CenterVertically){IconButton(back){Icon(Icons.Default.ArrowBack,null)};Column{Text(w.name,fontWeight=FontWeight.Bold);Text(w.client,color=Muted)}};ScrollableTabRow(page){listOf("Resumo","Tarefas","Diário","Checklist").forEachIndexed{i,s->Tab(page==i,{page=i}){Text(s)}}};when(page){0->Summary(w);1->ListPage("Tarefas",ts.map{it.title}, {dialog="task"}){i->vm.toggle(ts[i])};2->ListPage("Diário",ds.map{it.text},{dialog="diary"}){};else->ListPage("Checklist",cs.map{it.title},{dialog="check"}){i->vm.toggle(cs[i])}}};if(dialog.isNotBlank())TextDialog(dialog,{dialog="" ;when(dialog){"task"->vm.addTask(w.id,it);"diary"->vm.addDiary(w.id,it);"check"->vm.addCheck(w.id,it)}},{dialog=""})}}
